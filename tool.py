...
O = '\033[2;36m'
Le = pyfiglet.figlet_format('YaBh')
print(O+Le)

litters = 'qwertyuiopasd_.fghjklzxcvbnm1234567890'
id = input("[+] iD : ")
token = input("[+] ToKeN : ")
...
def Lev ():
	...
	while True:
	   ...
	   if '"statusCode":10221,' in reqsnd:
	           print(f'{user} 🛎️ ')
	           requests.post(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={id}&text=এ〔 𝑼𝒔𝒆𝒓𝒏𝒂𝒎𝒆 〕: {user}")
	   else:
	   	print(f'{user} 🔔 ')
Threads=[] 
for t in range(20):
